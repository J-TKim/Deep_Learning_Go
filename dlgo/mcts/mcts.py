...
from dlgo.agent.naive import Agent
...
class MCTSAgent(Agent):
    ...
    def select_move(self, game_state):
        ...
        for i in range(self.num_rounds):
            ...
            if node.can_add_child(): # 트리에 새 자식 노드 추가
                node = node.add_random_child()

            winner = self.simulate_random_game(node.game_state) # 이 노드에서 새 시뮬레이션

            while node is not None: # 트리를 거슬러 올라가며 점수 추가
